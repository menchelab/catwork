...
def networkGraphRT(myusers):
    ...
    G=nx.random_geometric_graph(len(annot),0.3)
    ...
    node_trace = go.Scatter(
        x=node_x, y=node_y,
        meta = nodes, # this can be used to pass custom info, here nodeIDs to the graph which can be retrived in js
        text = annot,
    ...
    fig.update_layout( autosize= True , paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=10, r=10, t=40, b=10))
...
def barGraph1(data):
    names = ["klaus","peter","erich"]
    val = [10,2,1]
    ids =[1,2,3]
    dat = pd.DataFrame({'word': names,
                        'counts': val } )
    fig = px.bar(dat, x = 'counts',y='word', orientation='h')
    fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", yaxis={'categoryorder':'total ascending'}) # add only this line
    return json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)



def barGraph(myusers):
    ...
    bar_height = 16*len(names)+200


    #fig.show()
    fig.update_layout(height = bar_height, autosize= True ,font_color = 'rgb(200,200,200)', paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=10, r=10, t=40, b=10))
    fig.update_yaxes(showticklabels=False)
    fig.update_layout(uniformtext_minsize=12, uniformtext_mode='show', yaxis={'categoryorder':'total ascending'})
    
    fig.layout.xaxis.fixedrange = True
    fig.layout.yaxis.fixedrange = True#print(fig["data"])
